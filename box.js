...
function GetCookie() {
if($request&&$request.url.indexOf("excitation_ad_treasure_box")>=0) {
   const jxcollecturl = $request.url
...
async function collect1(){
let urlo = jxcollecturl.split("?")[1]
 return new Promise((resolve) => {
    let collect_url1 = {
   		url: `https://api5-normal-c-lf.amemv.com/luckycat/aweme/v1/task/done/excitation_ad?${urlo}`,
        headers: JSON.parse(jxcollectheader)
   	}
   $.post(collect_url1,async(error, response, data) =>{
    try{
        const result = JSON.parse(data)
...
let urlo = jxcollecturl.split("?")[1]
 return new Promise((resolve) => {
    let collect_url2 = {
   		url: `https://api5-normal-c-lf.amemv.com/luckycat/aweme/v1/task/done/treasure_task?${urlo}`,
        headers: JSON.parse(jxcollectheader)
   	}
   $.post(collect_url2,async(error, response, data) =>{
    try{
        const result = JSON.parse(data)
        if(logs)$.log(data)
        if(result.err_no == 0)
